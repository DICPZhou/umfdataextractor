...
alphanumeric = R(r'^(d-)?(\d{1,2}[A-Za-z]{1,2}[′″‴‶‷⁗]?)(-d)?$')

# numeric = R('^\d{1,3}$')

letter_number = R(r'^(H\d)?[LSNM]{1,2}\d\d?$')

roman_numeral = R('^(I|II|III|IV|V|VI|VII|VIII|IX|XI|XII|XIII|XIV)$', re.I)

prefixed_label = R(r'^(cis|trans)-((d-)?(\d{1,2}[A-Za-z]{0,2}[′″‴‶‷⁗]?)(-d)?|[LS]\d\d?)$')

#: Particle label. Very permissive - must be used in context to avoid false positives.
strict_particle_label = (alphanumeric | roman_numeral | letter_number | prefixed_label)  # ('label')

lenient_particle_label = R(r'^([A-Z]\d{1,3})$') | strict_particle_label

label_type = (Optional(R('fresh|spent', re.I)) + Optional(
            R('Glass|magnetite|resin|sand|MCC|FCC|Gangue|Alumina|powder|bronze|cata|'
              'dolomite|char|copper|rubber|Silica|Red|quartz|olivine|calcined|manganese|'
              'polypro|molecu|steel|ceramic|plastic|shell|corn|γ|zirconia|bauxite|mullite|'
              'limestone|soya|bead|dolomite|iron|polystyrene|refractory|ballot|hollow|river|walnut|plastic|soga', re.I)
            + Optional(W('-'))
            + Optional(R('powder|gel|shot|balloons|bea|ballo|mud|sphere|dioxide|char|cob|sand|shell|Alumina|sieves|ore|material|dolomite|particle', re.I)))
              )('label').add_action(join) + Optional(colon).hide()

particle_label = ((Optional(label_type) + Optional(lenient_particle_label) + ZeroOrMore((T('CC') | comma) + Optional(lenient_particle_label))) |
                  (Optional(label_type.hide()) + strict_particle_label + ZeroOrMore((T('CC') | comma) + strict_particle_label)))('particle_label')
...
likely_abbreviation = (Optional(include_prefix + Optional(hyphen)) + R(r'^([A-Z]{2,6}(\-[A-Z]{1,6})?|[A-Z](\-[A-Z]{2,6}))$'))('name').add_action(join).add_action(fix_whitespace)
...
# class ChemicalLabelParser(BaseParser):  # parser of Heading in text.py
